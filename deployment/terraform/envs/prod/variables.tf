...
variable "enable_jenkins_workload_identity" {
  type        = bool
  description = "When true, creates Jenkins GCP SA + IAM in core-infra, and Kubernetes jenkins ServiceAccount + WI annotation + optional namespace admin RoleBinding."
  default     = true
}

variable "jenkins_gcp_sa_account_id" {
  type        = string
  description = "GCP service account_id for Jenkins; null uses module default jenkins-sa-<env>."
  default     = null
  nullable    = true
}
