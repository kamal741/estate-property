...
- After **`terraform apply`**, each env root (`envs/dev`, `envs/prod`) also creates Kubernetes Secrets in **`gke_namespace`** (unless you set **`create_app_runtime_secrets = false`** in `terraform.tfvars`):
  ...
  - If `kubectl get secret estateflow-admin-db -o jsonpath='{.data.username}' | base64 -d` prints `cG9zdGdyZXM=` instead of `postgres`, credentials were double-encoded: run **`../../../scripts/sync-app-runtime-secrets.sh <env>`** from `deployment/terraform/envs/<env>`, or from repo root **`./deployment/scripts/sync-app-runtime-secrets.sh <env>`**, or **`terraform apply -replace='kubernetes_secret_v1.estateflow_admin_db[0]'`** after pulling latest Terraform.
