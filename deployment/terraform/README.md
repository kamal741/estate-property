...
- After **`terraform apply`**, module **`modules/app-runtime-secrets`** creates Kubernetes Secrets in **`gke_namespace`** (unless **`create_app_runtime_secrets = false`** in `terraform.tfvars`). **New prod/dev stacks use `string_data` only** (plain text; the API stores a single base64 layer — do not use `data { base64encode(...) }`).
