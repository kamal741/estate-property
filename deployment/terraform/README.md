...
- After **`terraform apply`**, module **`modules/app-runtime-secrets`** creates Kubernetes Secrets in **`gke_namespace`** (unless **`create_app_runtime_secrets = false`** in `terraform.tfvars`). Secrets use **`data` with plain text** (hashicorp/kubernetes **>= 3.0** encodes once for the API — do not wrap values in `base64encode()`).
  - **`estateflow-admin-db`**: `username`, `password`, `host` (host matches Secret Manager `{env}-db-host`). Jenkins/`deploy.sh` read `host` for Helm `databaseHost`.
  - **`estateflow-redis`**: `host`, `password` (AUTH; host in `{env}-redis-host`).
- **Upgrading an env that already applied older secret resources** (resources moved into the module): from `deployment/terraform/envs/<env>` run once before apply:
  - `terraform state mv 'kubernetes_secret_v1.estateflow_admin_db[0]' 'module.app_runtime_secrets[0].kubernetes_secret_v1.estateflow_admin_db'`
  - `terraform state mv 'kubernetes_secret_v1.estateflow_redis[0]' 'module.app_runtime_secrets[0].kubernetes_secret_v1.estateflow_redis'`
  - Or let apply replace secrets (brief credential rotation risk) or run **`./deployment/scripts/sync-app-runtime-secrets.sh <env>`** after apply.
- If `kubectl get secret … | base64 -d` still shows nested base64 (e.g. `MTAuMzAuMC4z` instead of `10.30.0.3`), run **`./deployment/scripts/sync-app-runtime-secrets.sh <env>`** or **`terraform apply -replace='module.app_runtime_secrets[0].kubernetes_secret_v1.estateflow_admin_db' -replace='module.app_runtime_secrets[0].kubernetes_secret_v1.estateflow_redis'`**.
- These match the **`secretKeyRef`** names used by the **estateflow-admin-service** Helm chart. If **`apply`** errors with “already exists”, delete the existing secret once (`kubectl delete secret …`) or **`terraform import`** into the module resources, then apply again.
